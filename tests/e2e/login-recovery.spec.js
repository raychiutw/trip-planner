...
test('password has its own name and keyboard login recovers to the original destination',async({page})=>{
 ...
 await expect(password).toHaveAttribute('autocomplete','current-password');await email.fill('user@example.com');await email.focus();await page.keyboard.press('Tab');await expect(page.getByRole('link',{name:'忘記密碼？'})).toBeFocused();await page.keyboard.press('Tab');await expect(password).toBeFocused();await password.fill('a-password');await page.keyboard.press('Enter');
...
test('lockout has an announced heading and usable reset link',async({page})=>{
 await page.route('**/api/**',route=>route.fulfill({json:{}}));
 ...
 await expect(page.getByRole('heading',{name:'登入嘗試太多次'})).toBeFocused();await expect(page.getByRole('timer')).toBeVisible();await page.keyboard.press('Tab');await expect(page.getByRole('link',{name:'重設密碼'})).toBeFocused();
});
