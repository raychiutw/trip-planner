...
test('date dialog keyboard lifecycle, failed shift and read-only refresh recovery',async({page})=>{
 ...
 await opener.press('Enter');dialog=page.getByTestId('edit-trip-shift-modal');await dialog.getByRole('button',{name:'變更出發日期'}).click();await dialog.getByRole('button',{name:/May 3rd,/}).click();
