...
test('verification requires a gesture, recovers from network error, and keeps success until login is chosen',async({page})=>{
 let calls=0;
 ...
 await page.getByRole('button',{name:'點此完成驗證'}).focus();await page.keyboard.press('Enter');await expect(page.getByRole('alert')).toBeFocused();await expect(page.getByRole('alert')).toContainText('網路連線錯誤');await page.keyboard.press('Tab');await expect(page.getByRole('button',{name:'重試'})).toBeFocused();await page.keyboard.press('Enter');
 await expect(page.getByRole('status')).toContainText('Email 驗證成功');await page.clock.fastForward(5000);await expect(page).toHaveURL(/\/auth\/verify-email/);await page.keyboard.press('Tab');await expect(page.getByRole('link',{name:'前往登入'})).toBeFocused();await page.keyboard.press('Enter');await expect(page).toHaveURL(/\/login\?verified=1$/);expect(calls).toBe(2);
});
test('resend reports server failure, permits retry and honors server cooldown without a live countdown',async({page})=>{
 let calls=0;
 await page.route('**/api/**',route=>route.fulfill({json:{}}));
