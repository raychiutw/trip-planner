...
describe('FlightsSection — Edit', () => {
  ...
  it('v2.34.46: Edit airline + blur → autosave PATCH 直接觸發 with single field + expectedVersion', async () => {
    const onChange = vi.fn();
  ...
  it('v2.34.46: no-change blur → no PATCH（dirty check 早 return）', async () => {
    render(<FlightsSection tripId="trip-1" items={[mkFlight()]} onChange={vi.fn()} />);
    fireEvent.click(screen.getByTestId('flight-row-1').querySelector('.tp-notes-flight-body')!);
    const input = screen.getByTestId('flight-input-airline-1');
    fireEvent.blur(input);
  ...
  it('v2.34.46: 完成 button 已移除 — edit mode 只剩刪除 button', () => {
    render(<FlightsSection tripId="trip-1" items={[mkFlight()]} onChange={vi.fn()} />);
    fireEvent.click(screen.getByTestId('flight-row-1').querySelector('.tp-notes-flight-body')!);
    expect(screen.queryByTestId('flight-close-edit-1')).toBeNull();
    expect(screen.getAllByTestId('flight-delete-1').length).toBeGreaterThan(0);
  });
});
