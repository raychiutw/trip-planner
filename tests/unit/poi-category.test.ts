...
describe('Google Places primaryType → poi_type accuracy (add-stop auto-category fix)', () => {
  ...
  it('lodging types → hotel', () => {
    ...
    expect(mapNominatimCategory('bed_and_breakfast')).toBe('hotel'); // v2.55.73: b&b 是 lodging（原誤歸 attraction）
...
describe('poiCategoryLabel — 顯示用中文 label（含拉丁字母才映射；純中文原樣顯示）', () => {
  ...
  it('generic / 8 粗類值 primaryType → 中文（whitelist 或細類表收錄，不外露英文）', () => {
    expect(poiCategoryLabel('restaurant')).toBe('餐廳'); // 8 粗類 whitelist 值
    expect(poiCategoryLabel('other')).toBe('其他');
    expect(poiCategoryLabel('tourist_attraction')).toBe('景點'); // 細類表 generic 收錄
    expect(poiCategoryLabel('lodging')).toBe('飯店');
    expect(poiCategoryLabel('point_of_interest')).toBe('景點');
  });

  it('v2.55.73 有辨識度 primaryType → 細類 label（直用 Google 原始分類）', () => {
    // 過去這些全壓成 8 粗類（cafe→餐廳、subway_station→交通…）→ 現直用細類。
    expect(poiCategoryLabel('cafe')).toBe('咖啡廳');
    expect(poiCategoryLabel('ramen_restaurant')).toBe('拉麵');
    expect(poiCategoryLabel('sushi_restaurant')).toBe('壽司');
    expect(poiCategoryLabel('fast_food_restaurant')).toBe('速食'); // 真實資料：A&W
    expect(poiCategoryLabel('shopping_mall')).toBe('購物中心');
    expect(poiCategoryLabel('department_store')).toBe('百貨公司');
    expect(poiCategoryLabel('subway_station')).toBe('地鐵站');
    expect(poiCategoryLabel('aquarium')).toBe('水族館');
    expect(poiCategoryLabel('shinto_shrine')).toBe('神社');
    expect(poiCategoryLabel('buddhist_temple')).toBe('寺廟');
    expect(poiCategoryLabel('art_gallery')).toBe('美術館');
    expect(poiCategoryLabel('guest_house')).toBe('民宿');
  });

  it('補回中文：盤點 prod 全 343 POI 後，原本落英文 humanize 的 16 種值 → 中文細類', () => {
    // 2026-07-17：pois.category 回填（Google primaryType）後盤點，這 16 種原本走
    // humanizePrimaryType 顯示英文（Noodle shop / Ferry service…）。補進細類表。
    // 餐飲
    expect(poiCategoryLabel('tonkatsu_restaurant')).toBe('豬排');
    expect(poiCategoryLabel('yakiniku_restaurant')).toBe('燒肉');
    expect(poiCategoryLabel('noodle_shop')).toBe('麵店');
    expect(poiCategoryLabel('japanese_curry_restaurant')).toBe('日式咖哩');
    // 景點・文化
    expect(poiCategoryLabel('scenic_spot')).toBe('觀景點');
    expect(poiCategoryLabel('castle')).toBe('城堡');
    expect(poiCategoryLabel('island')).toBe('島嶼');
    expect(poiCategoryLabel('tourism')).toBe('觀光景點');
    // 交通
    expect(poiCategoryLabel('parking_lot')).toBe('停車場');
    expect(poiCategoryLabel('ferry_service')).toBe('渡輪');
    expect(poiCategoryLabel('railway')).toBe('鐵路');
    expect(poiCategoryLabel('transportation_service')).toBe('交通服務');
    // 活動
    expect(poiCategoryLabel('sports_activity_location')).toBe('運動場所');
    expect(poiCategoryLabel('tour_agency')).toBe('旅行社');
    // 其他泛用
    expect(poiCategoryLabel('service')).toBe('服務據點');
    expect(poiCategoryLabel('corporate_office')).toBe('辦公室');
    // 補回後全域不該再有這些值落英文（掃整包 label，無 ASCII 拉丁）
    for (const v of ['noodle_shop', 'ferry_service', 'castle', 'tour_agency']) {
      expect(/[a-zA-Z]/.test(poiCategoryLabel(v) ?? '')).toBe(false);
    }
  });

  it('v2.55.73 細類表未命中的英文 primaryType → 顯示英文（Title Case，事後補救）', () => {
    // 使用者要求：沒對應到中文的就顯示英文，之後再補 mapping。純 snake_case 英文
    // 未收錄 → 人性化英文（底線轉空格、字首大寫），讓缺漏可見。
    expect(poiCategoryLabel('hindu_temple_annex_xyz')).toBe('Hindu Temple Annex Xyz');
    expect(poiCategoryLabel('CAFE')).toBe('咖啡廳'); // case-insensitive 命中細類表 → 中文
    // 混合中英 / 雜訊仍走乾淨粗類，不外露（見下方既有 junk / 混合字串測試）。
    expect(poiCategoryLabel('123')).toBe('景點');
  });
