/**
 * Unit test — tokens.css .ocean-bottom-nav sticky 定位（-4.2）
 *
...
describe('tokens.css — .ocean-bottom-nav sticky positioning ', () => {
  it('.ocean-bottom-nav 用 position: sticky', () => {
    expect(TOKENS).toMatch(/\.ocean-bottom-nav\s*\{[\s\S]*?position:\s*sticky/);
  });

  it('.ocean-bottom-nav 用 inset-block-end: 0', () => {
    expect(TOKENS).toMatch(/\.ocean-bottom-nav\s*\{[\s\S]*?inset-block-end:\s*0/);
  });

  it('.ocean-bottom-nav 不再使用 position: fixed', () => {
  ...
  it('.ocean-bottom-nav 保留 padding-bottom: env(safe-area-inset-bottom)', () => {
