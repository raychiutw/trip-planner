...
describe('TravelPill — v2.29.2 stale-travel ⚠ (computed_at signal)', () => {
  it('segment 缺席（純 legacy travel_*）→ 不渲染 ⚠（沒 segment 表示 no stale signal source）', () => {
  ...
  it('segment.computedAt 有值（Google Routes 算過）→ 不渲染 ⚠，顯示正常 min/dist', () => {
    render(<TravelPill segment={seg({ computedAt: 1700000000000 })} tripId="t1" />);
    expect(screen.queryByTestId('travel-pill-stale')).toBeNull();
    expect(screen.queryByText('15 min')).toBeTruthy();
  });

  it('segment.computedAt = null（backend mark stale）→ 渲染 ⚠ + 清空 min/dist', () => {
  ...
  it('⚠ chip aria-label 提到 Google Routes 尚未重新計算', () => {
    render(<TravelPill segment={seg({ computedAt: null })} tripId="t1" />);
    const stale = screen.getByTestId('travel-pill-stale');
    expect(stale.getAttribute('aria-label') ?? '').toMatch(/Google\s*Routes|未更新/i);
  });

  it('「重新計算」button 點擊觸發 onRecompute', () => {
    const onRecompute = vi.fn();
    render(
      <TravelPill
        segment={seg({ computedAt: null })}
        tripId="t1"
  ...
  it('stale 但無 onRecompute → 渲染 ⚠ chip 但無 recompute button', () => {
    render(<TravelPill segment={seg({ computedAt: null })} tripId="t1" />);
    expect(screen.getByTestId('travel-pill-stale')).toBeTruthy();
