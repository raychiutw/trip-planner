 ...
 *
 * 驗證 tokens.css 在 @media (max-width: 760px) 下的 .tp-hero-title
...
describe('F003 — Mobile Type Scale 真正落地', () => {
  it('tokens.css 在 @media (max-width: 760px) 下 .tp-hero-title 使用 24px', () => {
    // 找到 760px 媒體查詢區塊內的 tp-hero-title font-size
    ...
    // 在 760px 媒體查詢段落中找到 tp-hero-title: 24px
    const has24pxHeroTitle = mobileMediaMatch?.some(
      (block) => block.includes('.tp-hero-title') && block.includes('24px'),
    );
