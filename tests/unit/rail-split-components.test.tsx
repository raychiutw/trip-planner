...
describe('RailRow（獨立 render）', () => {
  ...
  it('正選更換時隔離尚未送出的備註，重新編輯只寫新的正選', async () => {
    vi.useFakeTimers();
    try {
      ...
      fireEvent.change(screen.getByTestId('timeline-rail-note-input-42'), { target: { value: '舊景點的備註' } });
      view.rerender(row({ ...entry, note: '新景點原有備註', stopPois: [
        { poiId: 9002, sortOrder: 1, name: '泊港漁市場' },
        { poiId: 9001, sortOrder: 2, name: '糸滿魚市場' },
      ] }));
      await act(async () => { await vi.advanceTimersByTimeAsync(800); });
      expect(mutations.updateEntryPoi).not.toHaveBeenCalled();
      expect(screen.queryByTestId('timeline-rail-note-input-42')).toBeNull();
      fireEvent.click(screen.getByTestId('timeline-rail-note-value-42'));
      expect(screen.getByTestId('timeline-rail-note-input-42')).toHaveValue('新景點原有備註');
      fireEvent.change(screen.getByTestId('timeline-rail-note-input-42'), { target: { value: '新景點的新備註' } });
      await act(async () => { await vi.advanceTimersByTimeAsync(800); });
      expect(mutations.updateEntryPoi).toHaveBeenCalledTimes(1);
