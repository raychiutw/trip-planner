 ...
 * 歷史 bug：trip_entries 排在 pois 前 → trip_entries 會 import 0 rows。
 *
 * v2.29.0: trip_pois table DROPPED；trip_entries.poi_id DROPPED；
 * hotel 改 trip_days.hotel_poi_id → trip_days 必須在 pois 之後。
 */
...
describe('init-local-db.js TABLES FK-safe order', () => {
  ...
  it('pois 必須在 trip_days 之前（FK: trip_days.hotel_poi_id → pois.id, migration 0060）', () => {
    expectParentBeforeChild(tables, 'pois', 'trip_days');
  });
