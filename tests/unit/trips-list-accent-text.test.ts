 ...
 * WCAG 1.4.11 把 pure decoration 列為**明文例外**（不是「適用但門檻降到 3:1」）—— 兩者旁邊
 * 都另有文字標籤，圖示本身不承載資訊，所以沒有任何對比門檻拘束它們。
 * 我們仍自訂 3:1 下限（實測 3.24:1）當安全邊際，理由見下面 aria-hidden 那條測試：
 * 一旦圖示不再是裝飾，1.4.11 就**真的**適用、門檻正好是 3:1，那時 3.24 才是唯一的緩衝。
...
describe('TripsListPage 柔褐文字對比 call-site 守衛（#1156）', () => {
  ...
  it('裝飾例外的前提仍成立：兩個圖示都還是 aria-hidden', () => {
    ...
    // 那是 WCAG 1.4.11 的明文例外，沒有門檻拘束。前提一垮（圖示開始承載語意），
    // 1.4.11 立刻適用、門檻 3:1，而現值只有 3.24、餘裕 0.24。
