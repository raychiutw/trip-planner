...
describe('AppShell — CSS rules（驗 SCOPED_STYLES，jsdom 不套 media query）', () => {
  ...
  it('mobile sets explicit grid height so main is a real scroll container', () => {
    // Bottom-nav is now in its own grid row (auto), not overlapping main —
    // so .app-shell needs height: 100dvh (not just min-height) for main to
    // be constrained and scrollable. Otherwise document scrolls and sticky
    // elements inside main lose their context.
    expect(APP_SHELL_STYLES).toMatch(
      /@media[^{]*max-width:\s*1023px[\s\S]*?\.app-shell\b[^{]*\{[^}]*?height:\s*100dvh/,
    );
  });
