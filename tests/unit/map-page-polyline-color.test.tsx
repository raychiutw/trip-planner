...
describe('MapPage polyline 顏色 (map-page-multiday-overview)', () => {
  ...
  it('MapPage 傳 dayNum 或 pinsByDay prop 給 TpMap', () => {
    // 期望 <TpMap ...> JSX 含 dayNum 或 pinsByDay prop
    const match = src.match(/<TpMap[\s\S]*?(dayNum|pinsByDay)/);
    expect(match, 'MapPage 應傳 dayNum 或 pinsByDay prop 給 TpMap').not.toBeNull();
  });
});
