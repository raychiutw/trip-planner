...
### Page Titlebar（`TitleBar` — 唯一 page chrome primitive）

**2026-05-03 PageHeader 退役**：原本 PageHeader 用於 settings 子頁（`ConnectedAppsPage` / `DeveloperAppsPage` / `SessionsPage`），現全部統一為 TitleBar。eyebrow + meta 改用 inline `.tp-page-eyebrow` + `.tp-page-meta` 在 TitleBar 下方第一行（保留資訊密度）。`src/components/shell/PageHeader.tsx` 已刪。
...
**Sticky chrome zone（2026-05-03 audit 文件化 — 解釋 user 「明細 TitleBar 較高」 視覺感知）：**

部分 page 的 TitleBar 後緊接 sub-toolbar 形成「sticky chrome group」，視覺上跟 TitleBar 連在一起變成多 row chrome：

| Page | Chrome group 結構 | TitleBar 後 spacing |
|------|---|---|
| TripPage（embedded mode） | TitleBar 56px + DayNav `.ocean-day-strip` (sticky `top: 64px`, negative margin 貼附) | `0` — by design 形成 sub-toolbar |
| MapPage | TitleBar + day tab strip (bottom underline tabs sticky) | `0` — 同 sticky group |
| TripsListPage 一覽 | TitleBar + `.tp-trips-inner` content | 24px buffer (mobile/desktop) |
| ChatPage | TitleBar + `.tp-chat-body` | 16-20px buffer |
| ExplorePage | TitleBar + `.explore-wrap` | 16-24px buffer |
| AccountPage | TitleBar + `.tp-account-inner` | 24-40px buffer |
| ConnectedApps / DeveloperApps / Sessions | TitleBar + `.tp-page-eyebrow` (zero margin-top) + content | 0 — by design eyebrow 緊接 TitleBar |
| AddStop / EditTrip / NewTrip / EntryAction / DeveloperAppNew | TitleBar + `.tp-*-page-shell` content | per-page 16-24px buffer |

**規則**：TitleBar 自身 height 永遠 64px / 56px；後續 element 是否貼附（0 buffer）是 **page-level decision**，遵循 design intent — DayNav-style sub-toolbar 該貼，content section 應有 16-24px buffer。**不要把 sub-toolbar 緊貼當作 TitleBar height 的一部分**（CSS 上是兩個獨立 sticky element）。
...
| 2026-05-03 | **TitleBar 後 spacing buffer audit — sticky chrome zone 文件化** | User 觀察「行程明細 TitleBar 較高」。Audit 全 page TitleBar 後 spacing — 發現 TripPage embedded mode 的 DayNav 用 `margin: -28px -40px 20px` + sticky `top: 64px` 故意貼附 TitleBar 形成 sticky chrome group (類 iOS TabBar + sub-toolbar pattern)，是 **design intent** 不是 bug。TitleBar 自身 height 永遠 64px / 56px 跟其他 page 一致。文件化「Sticky chrome zone」 規則於 DESIGN.md Page Titlebar section + per-page spacing audit table，明確區分「TitleBar height (CSS fixed)」 vs 「chrome group 視覺高 (含 sub-toolbar)」。無 code change — design intent 確認後不需修。 |
| 2026-05-03 | **PageHeader 退役 — 統一所有 page 用 TitleBar** | User 觀察:「page header 是什麼？應該都要用 title bar 才對」。原本分歧：6 主功能頁 + form 全頁 5 page + Account/Appearance/Notifications 用 TitleBar；ConnectedApps / DeveloperApps / Sessions 3 個 settings 子頁用 PageHeader (有 eyebrow + meta + actions)。3 page swap 為 TitleBar，eyebrow + meta 改用 inline `.tp-page-eyebrow` + `.tp-page-meta` 在 TitleBar 下方（新加 CSS helper 保留資訊密度）。`src/components/shell/PageHeader.tsx` + `tests/unit/page-header.test.tsx` 刪除，`css/tokens.css` 移除整段 PageHeader CSS section (~169 行) 並加上 page-eyebrow/meta helper。所有 page 現在統一 chrome：TitleBar 唯一規範,height 桌機 64px / 手機 56px,actions 用 `.tp-titlebar-action` (default outline) 或 `.is-primary` (accent filled)。 |
