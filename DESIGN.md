...
- **Touch target:** 最小 44×44px (Apple HIG)
  - Exception: drag handles (e.g. `.tp-rail-grip`) 視覺 24×24px — 跟 row 主點擊區並存時避免 click target 衝突。a11y 補償：`:focus-visible` ring + 桌機淡顯 opacity 0.4（hover→1）。
  - 觸控裝置（`@media (hover: none)`，無 hover）：把手常駐 opacity 1 +`::before` inset hit-slop 撐到 44×44px 觸控目標（視覺仍 24px、不動 grid），避免手機找不到/按不到把手（2026-07-08 真機實測修正；hit-slop 只溢入非互動的時間欄，不碰 row 主點擊區）。
