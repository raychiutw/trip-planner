...
**Action button responsive 規則（2026-05-03）：**
- **桌機**（`min-width: 1024px` 且 `pointer: fine`）：action button = `icon + 文字`，例如「💾 儲存」「✓ 完成」
  - Icon 14-16px + 文字 14px 同行；padding `8px 16px`；min-height 36px
  - 多 action 時水平排列，主要 action（accent 實心）放最右
- **手機 / compact**（`max-width: 1023px` 或 coarse pointer）：action button = `icon only`
  - 36×36 圓形或方形 button；icon 18-20px；文字隱藏（用 `aria-label` 給螢幕閱讀器）
  - 多 action 時水平排列，主要 action 仍 accent 實心
- **轉換實作**：CSS class `.tp-titlebar-action-label` 在 compact 改 `display: none`，桌機 `inline`。或元件層用 `useMediaQuery('(min-width: 1024px)')` 條件 render。
...
| 現況 | 違規原因 | 遷移目標 | 狀態 |
|---|---|---|---|
| `NewTripModal.tsx` | 9+ 欄位 + sortable destinations + POI search dropdown + day quota stepper | → `/trips/new` page | 🔴 待遷 |
| `EditTripModal.tsx` (v2.19.0) | 6+ 欄位 + sortable destinations + region change hint + segment + toggle | → `/trip/:id/edit` page | 🔴 待遷 |
| `AddStopModal.tsx` | POI search + multi-section form | → `/trip/:id/add-stop` page | 🔴 待遷 |
...
| `EntryActionPopover.tsx` | popover menu — 符合 popover 規則 | — | ✅ 保留 |
