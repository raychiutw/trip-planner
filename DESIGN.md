...
### saved_pois universal pool

- **單一收藏池**：每個 user 一個 `saved_pois` 池。`(user_id, poi_id)` UNIQUE — 不能重複收藏同 POI。
- **跨 trip 反查**：透過 `saved_pois.poi_id ← trip_pois.poi_id` JOIN，每筆收藏即時可知「目前在哪些 trip / 哪天 / 哪 entry 出現」。後端 GET 用 `json_group_array` 一次查（避 N+1），usages 隨 saved POI row 一起回。
- **「目前在 N 個行程」徽章**：Saved POI card 在 POI 名稱下方加一行 `--font-size-footnote --color-muted` 文字，內容例「目前在 3 個行程」。N=0 時不渲染（避免「0 個行程」噪音）。
- **進行程不刪收藏**：「搬」 = 複製；saved_pois 是「跨 trip 願望清單」，進行程不代表不想再去（同景點可能不同 trip 都想去）。

### 加入行程 fast-path（route `/saved-pois/:id/add-to-trip`）

**Page (full)，不是 modal** — DESIGN.md L390-414 form 規範：>3 fields + select + time picker 必走全頁。檔名 `AddSavedPoiToTripPage.tsx`。

**Form fields (6)**：
1. trip dropdown（user 只 1 個 trip 自動選）
2. day dropdown（依選定 trip 動態載入 days）
3. position：`append` / `before` / `after` / `replace`（預設 append）
4. anchor entry id（position ≠ append 時必填）
5. start time（可空，依 POI type 推 — restaurant 12:00 / attraction 10:00 / hotel 15:00）
6. end time（可空，預設 = start + stay duration heuristic）
...
**Endpoint**：`POST /api/saved-pois/:id/add-to-trip`。Fast-path REST，**不走** message-based tp-request — 避免 LLM 8 秒等待感。`travel_*` 欄位 NULL，背景 tp-request 之後 fill。UI 顯示「計算車程中…」placeholder。
...
> **v2.22.0 rename**：原 `SavedPoisPage` / `/saved` 已 rename 為 `PoiFavoritesPage` / `/favorites`（migration 0050）。下方規範保留歷史描述但 page name / route 已對齊新名。

「收藏」自 v2.21.0 升為第 4 個 primary nav slot（取代「探索」）。`PoiFavoritesPage` 為新建獨立 route `/favorites`，從 ExplorePage L887-1011 saved-view section 抽出且擴展為 top-level 頁面。
