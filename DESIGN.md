...
**地圖 chrome 子例外（2026-04）：** 在地圖相關頁面（`MapPage`、`TripMapRail`、`OceanMap`）的 **Day 指示 tab active state**（底線色、eyebrow `DAY NN` 文字色），可套用 `dayColor(N)` 作為 visual cueing，與 polyline 同色呼應。理由：使用者需要能快速辨認「當前看的是哪一天」，chrome 色和 polyline 色對齊提供一致的心智模型。適用範圍**僅限 Day 指示 tab**；其他 chrome（返回鈕、主導航、title、overflow menu）仍嚴守 Ocean。
...
- **hotel marker 樣式不變**：仍用 ink（`#222222`）類 stop 顏色（per Stop Type Color Convention），只有 polyline 把它包進來。
