...
- **Compact shell:** sticky page titlebar + right-side hamburger menu + bottom nav。**底部膠囊常駐**（owner 2026-07-20 決定；不實作 HIG 的 `tabBarMinimizeBehavior`，HIG 本就允許 `.never`）—— 4-tab 是 global IA 主導覽，常駐比省 58px 垂直空間重要。
...
- **Trip detail DayNav:** sticky 在 titlebar 下方，行為與 bottom nav 一致：**常駐不隱藏**。
...
- **Glass — 單一 Regular Glass 材質（2026-07-20）:** 全站 chrome（Titlebar、bottom-nav 膠囊、page bottom bar、sheet、stack panel header、chat composer）吃**同一組六個材質 token**：`--glass-tint` / `--glass-filter`(`blur(24px) saturate(180%)`) / `--glass-rim` / `--glass-specular` / `--glass-shadow` / `--chrome-inset`(21px)。退役 `--blur-glass` / `--color-glass-nav` / `--color-glass-toast` —— 單一 blur 值只統一了 Liquid Glass 六層裡的一層，其餘五層各自漂移，是 drift 的結構性根源。

  **鐵則：玻璃不上品牌 tint。** 禁止 `color-mix(in srgb, var(--color-background|secondary|accent) N%, transparent)` 當 chrome background。Apple HIG 明令「glass 不上 tint，顏色留給 content layer」，而 Regular 變體的定義是 「provides legibility **regardless of context**」—— 需要依背景分軌代表材質本身做錯了。奶油玻璃疊奶油頁必然 cream-on-cream（2026-07-20 實證：62%→72%→88% 全都沒用，根因是色相不是數值）。`--glass-tint` 是中性的，底層顏色由 `saturate(180%)` 抽上表面。

  ⚠ `--glass-tint` 的 alpha (0.80) 由**最壞情況對比度反推**，非美感值：膠囊可能浮在 satellite/hybrid 圖磚上（`MapFabs` `MapTileStyle`），而 CSS 沒有 HIG 的 adaptivity。深色 tint 80% 疊近白衛星 → 合成 #5E5E5E，配 `--color-foreground` = 5.92:1 ✅；0.62 只有 3.55:1 ❌（11px bold 不算 WCAG large text，門檻 4.5 非 3.0）。同理 bottom-nav inactive label 用 `--color-foreground` 而非 `--color-muted`，active/inactive 靠 accent 實心藥丸區分。

  **Token 放置規則（dark mode 靜默失效防線）**：隨主題變值的四個（tint/rim/specular/shadow）淺色值放 `@theme`、深色放 `@layer base { body.dark }`。**不可**放進 `tokens.css` 未分層的 `body {}` —— CSS 串接層規則是「未分層勝過任何 `@layer`」，放錯會讓 `body.dark` 覆寫永不生效，且無測試抓得到。契約由 `tests/unit/glass-tokens.test.ts` 鎖住。

  **已核可例外**（各有測試守護，不受 Regular Glass 約束）：① **桌機 sidebar vibrancy**（§10.1）：`DesktopSidebar` 是 macOS NSVisualEffectView 式 **vibrancy 材質**（非 chrome glass），`blur(30px) saturate(180%)`，**且刻意保留品牌 tint** —— 它是唯一的例外，後續 PR 不要順手「統一」掉。② **Small floating button**：≤32px 浮動按鈕（POI 卡 cover 上的 `+ 加入` / `⋯` / `❤`）用 `blur(8px)` —— 24px 在小元素上會 over-soften 邊緣。③ **扁平卡（無 cover 圖）的 `⋯`**（`TripsListPage` `TripCardMenu`）：**不做玻璃圓** — resting 透明只留字符、hover/focus 才淡底（44px tap 區保留）；實心玻璃圓在奶油卡上與卡片圓角相撞、像貼紙破版。
...
- **Shadow specialized 例外**：chrome 的浮起與邊緣高光由 `--glass-shadow`（雙層落影）與 `--glass-specular`（inset 上緣高光）承載 —— 它們是 box-shadow **值** token，不進下方 Airbnb 三層 scale 表，也不需要逐處 inline。除此之外，地圖 markers 的 active state ring + drop shadow 可用 inline 多層 shadow（如 `0 0 0 4px rgba(217,120,72,0.3), 0 4px 12px rgba(42,31,24,0.45)`），因為 marker 浮在地圖背景上需要更強對比，token 三層 shadow 過弱。例外限定地圖 marker，其他 UI 一律用 token。
...
- Sticky top 0 + **HIG scroll edge effect**：捲到頂＝`background: transparent` + border 透明（標題直接坐在內容上）；捲動後 `.is-scrolled` 掛上 Regular Glass + hairline，200ms 淡入。border 保留 1px 只改顏色，避免首次捲動整頁跳 1px；transition 不含 `backdrop-filter`（逐 frame 重算會 jank）。
- **無捲動容器的頁面需 `alwaysSolid`**：MapPage 是 full-bleed、`.map-page-wrap` 永不捲動，沒有 opt-out 會卡在透明態、標題壓在圖磚上（衛星影像時不可讀）。
- scroll edge 綁**捲動祖先**（`lib/scrollAncestor`），不綁 window —— `.app-shell{height:100dvh}` 保證 document 永不捲動，綁 window 是全站死碼且不會有測試抓到。
...
- **常駐不隱藏**（owner 2026-07-20）。原本的 scroll-direction-aware 隱藏（向下捲 `translateY(180%)` / 向上捲回）已整套移除，`AppShell` 不再持有 scroll direction state。
...
- 共用 wrapper：horizontal scroll + scrollbar hidden + Regular Glass 材質 token（`--glass-tint`/`--glass-filter`/`--glass-rim`/`--glass-specular`+`--glass-shadow`）
...
- **body scroll-lock**（`modal` 預設 true；桌機 non-modal 右欄操作面板傳 `modal:false` 不鎖，避免凍結中欄）。**ref-count**（`useBodyScrollLock` module-level 計數）→ 巢狀 sheet 共用一把鎖，內層不覆寫外層捲動位置。
...
- **Chrome 材質降級（2026-07-20）:** Apple 在 iOS 上是系統自動套用，Web 必須手寫。三個 media query 對應 HIG 三個系統設定，實作在 `css/tokens.css`（**刻意未分層**，才能同時蓋掉 `@theme` 的淺色值與 `@layer base` 的 `body.dark` 值）：
  | 系統設定 | HIG 行為 | 本專案 |
  |---|---|---|
  | `prefers-reduced-transparency: reduce` | 玻璃更霜化，遮蔽更多背後內容 | `--glass-tint` → `rgba(255,255,255,.94)` / dark `rgba(44,44,46,.96)`；`--glass-filter` → `blur(8px)` |
  | `prefers-contrast: more` | 元素變近黑白 + 對比色邊框 | `--glass-tint` → `var(--color-background)` 全不透明；`--glass-filter` → `none`；`--glass-rim` → `2px solid var(--color-foreground)`；specular 關閉 |
  | `prefers-reduced-motion` | 停用彈性屬性 | scroll edge 的 200ms 材質淡入改直接切換 |
  另有 `@supports not (backdrop-filter: blur(1px))` 給不透明地板 —— 0.80 tint 沒有 blur 撐著，疊在 satellite 圖磚上文字會不可讀，寧可失去玻璃感也不能失去可讀性。
