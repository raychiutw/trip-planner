...
歷史決策摘要（一行式）。詳細實作背景見 `git log` + PR descriptions。

| Date | Decision |
|------|----------|
| 2026-05-03 | TitleBar action button 統一規範：`.tp-titlebar-action` outline pill + `.is-primary` accent filled，桌機 icon+文字 / 手機 icon-only round + 1px border 兩 breakpoint 一致 |
| 2026-05-03 | `.tp-embedded-trip` layout 從 flex column 改 grid `auto + 1fr` (chrome + main pattern)，避免 TitleBar 在 flex container 被 squeeze |
| 2026-05-03 | PageHeader 退役 — 統一所有 page 用 `<TitleBar>`，eyebrow + meta 改用 `.tp-page-eyebrow` + `.tp-page-meta` inline |
| 2026-05-03 | Modal-to-fullpage migration 完成 — 6 個違規 modal (NewTrip / EditTrip / EntryAction / AddStop / DeveloperApp create / ExplorePage trip-picker) 全數遷移；P3 confirm-modal cleanup 收尾用標準化 `<ConfirmModal>` |
| 2026-05-03 | Modal vs Full Page 明文規則：Modal 限定 confirm / input / popover / critical attention；form 流程 / sortable / inline search 必走全頁；chooser 走 popover |
| 2026-05-03 | Sticky chrome zone 文件化：TitleBar height 永遠 fixed (64/56px)，後續 sub-toolbar 是否貼附是 page-level decision |
| 2026-05-02 | TimelineRail row 改單層 5 欄 grid `24 60 44 1fr 24`(grip / time+dur / icon / content / caret) |
| 2026-05-02 | Grip handle 24×24 例外 (vs Apple HIG 44×44)，避免跟 row 主點擊區搶 click target |
| 2026-05-02 | 餐廳推薦 section (meal entry expand 內) hero variant + standard cards |
| 2026-05-02 | EntryActionPopover responsive: 寬度 `min(320px, calc(100vw - 32px))` + Day label 兩行 stack |
| 2026-05-02 | MapPage 預設「總覽」tab (無 `?day=` 參數時) |
| 2026-04-27 | Terracotta pages refactor: TitleBar 統一 main chrome，PageHeader 只給 settings/splash (此規則於 2026-05-03 廢除) |
| 2026-04-27 | Trip detail desktop/compact 共用內容；compact bottom nav active 依子頁所屬主功能 |
| 2026-04-27 | Persistent error messaging — Toast 限低風險狀態，重要錯誤用 persistent surfaces |
| 2026-04-27 | Desktop / compact typography split — 桌機 vs compact role-based type scale |
| 2026-04-27 | Unified app chrome + Terracotta source of truth — 5 主功能 page 統一 titlebar/sidebar/bottom-nav |
| 2026-04-23 | 移除 `TriplineLogo` — 全站 header 不顯示品牌標誌 |
| 2026-04-21 | Glass unified blur(14px) 三層一致；DV color exception (10 色 qualitative palette) |
| 2026-04-19 | Stop card 4-col grid + hairline borders + stop type accent (only sight/food) |
| 2026-04-19 | Ocean single-theme redesign (已由 2026-04-27 Terracotta 取代) |
| 2026-04-25 | Layout Refactor v2.3.0 — Mindtrip-inspired 3-pane shell + URL-driven sheet state + ARIA tabs + WCAG AA + reduced-motion override |
| 2026-03-29 | DESIGN.md created — Tailwind CSS 4 + tokens.css + Apple HIG type scale |
