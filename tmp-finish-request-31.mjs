// 完成 Request #31 的剩餘操作
// 網路恢復後執行: node tmp-finish-request-31.mjs
...
async function api(method, path, body) {
  ...
  if (res.status >= 400) console.log('  ERROR:', text.substring(0, 200));
  return res.status;
}

async function main() {
  // 1. PATCH entry 374 note — 加入 plate jam 備案資訊
...
| 20:11 | BUZZ RESORT Check in |`;
  ...
  await api('PATCH', '/api/requests/31', {
    reply,
    status: 'closed',
