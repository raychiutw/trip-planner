...
export async function verifyRestaurantBelongsToTrip(
  ...
  const row = await db
    .prepare('SELECT 1 FROM restaurants r JOIN trip_entries e ON r.entry_id = e.id JOIN trip_days d ON e.day_id = d.id WHERE r.id = ? AND d.trip_id = ?')
...
export async function verifyHotelBelongsToTrip(
  ...
  const row = await db
    .prepare('SELECT 1 FROM hotels h JOIN trip_days d ON h.day_id = d.id WHERE h.id = ? AND d.trip_id = ?')
    .bind(hotelId, tripId)
    .first();
  return !!row;
}
