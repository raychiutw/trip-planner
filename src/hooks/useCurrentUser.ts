 ...
 * state（error 表示可重試的未知狀態），不要先當成未登入，避免 auth-dependent chrome flicker。
 *
 * 不快取 across navigation — useEffect fetch on mount。Future V2-P5 加 SWR-style
 * cache + revalidate（避免每頁 mount 都打 API）。
 *
 * 副作用：確認登入或未登入才會寫一個「上次是否已登入」的布林旗標到 localStorage
...
export interface UseCurrentUserResult {
  /** undefined = loading or failed probe (see error), null = unauthenticated, CurrentUser = logged in */
  user: CurrentUser | null | undefined;
  /** A failed probe is unknown, not an anonymous session. */
  error: boolean;
  ...
  useEffect(() => {
    ...
    fetch(USERINFO_ENDPOINT, { credentials: 'include', signal: controller.signal })
      .then(async (res) => {
        ...
        if (res.status === 401) {
          // Only an explicit authentication failure establishes anonymity.
          setUser(null);
          writeAuthHint(false);
          return;
        }
        if (!res.ok) throw new Error('userinfo unavailable');
        const data = (await res.json()) as CurrentUser;
        if (controller.signal.aborted) return;
        if (!data || typeof data.id !== 'string' || typeof data.email !== 'string') throw new Error('invalid userinfo');
        setUser(data);
