...
- **WHEN** 驗證任一行程 JSON
- **THEN** `meta` SHALL 不包含 `themeColor` 或 `name` 欄位（已移除）
- **AND** `meta.tripType` 驗證不變
...
#### Scenario: hotel.subs 新格式結構

- **WHEN** hotel 物件包含 `subs`
- **THEN** `subs` SHALL 為陣列，每個元素 SHALL 包含 `type`（字串）和 `title`（字串）
- **AND** 選填欄位：`price`（字串）、`note`（字串）、`location`（Location 物件）
- **AND** SHALL 不包含舊格式欄位 `label` 或 `text`
