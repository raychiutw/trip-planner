...
trip_entry_pois    trip_entries × pois junction；sort_order=1 是正選景點，
                   sort_order>1 是備選景點；entry_pois_version 做 OCC
...
用餐 stop 的餐廳 choice 仍在過渡期同時由 `trip_pois context='timeline'` surface。`GET /api/trips/:id/days/:num` 會回傳 `stopPois`；若 legacy 用餐 stop 的 stored master 仍是 wrapper POI，API response 會把第一順位餐廳 virtual promote 成 `poi/master/stopPois[0]`，讓一覽、地圖與詳細視窗都顯示實際正選餐廳。Migration 0059 可把這個狀態持久化回 `trip_entry_pois.sort_order=1`。
