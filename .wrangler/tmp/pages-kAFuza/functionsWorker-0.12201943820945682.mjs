...
var onRequestPut = /* @__PURE__ */ __name(async (context) => {
  ...
  stmts.push(
    db.prepare("UPDATE days SET date = ?, dayOfWeek = ?, label = ?, weather = ? WHERE id = ?").bind(
      body.date ?? null,
      body.dayOfWeek ?? null,
      body.label ?? null,
...
var onRequestGet4 = /* @__PURE__ */ __name(async (context) => {
  const { id } = context.params;
  const { results } = await context.env.DB.prepare("SELECT id, day_num, date, dayOfWeek, label FROM days WHERE trip_id = ? ORDER BY day_num ASC").bind(id).all();
...
// ../.wrangler/tmp/pages-kAFuza/functionsRoutes-0.10448045049173271.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-d2PwKw/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-d2PwKw/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.12201943820945682.mjs.map
