...
.claude/skills/     tp-check  tp-create  tp-edit  tp-request  tp-patch  tp-rebuild  tp-deploy  tp-code-validate  ...
...
- **命名規範驗證**：commit 前必須跑 `/tp-code-validate` 驗證命名規範綠燈，紅燈則持續修改直到通過；命名規範詳見 `openspec/config.yaml` naming 區塊
