...
兩個手動交通入口透過 `manualSegment` 共用 POST／PATCH 與成功後的 segment 刷新通知；已成功寫入的 segment 不因後續重讀失敗而重送。對話框沿用 autosave 與版本衝突重讀；entry 編輯仍可不帶版本，並分別記錄 entry 與 segment 的成功結果，單邊失敗時只重試未成功的部分。過期行程操作保留已送出的 server 寫入，但不發布目前畫面的通知或提示。
