...
### Fixed
- 儲存、搜尋、收藏批次操作、分享、列印與 PDF 匯出失敗時保留輸入與已完成結果，可明確重試；切換行程後，舊回應不再覆蓋目前畫面。
- 行程匯入與複製共用分段寫入及失敗補償；手動交通、景點搬移與排序同步目前行程的車程狀態。聊天、行程清單與地圖延續有效的行程選擇。
- 探索分頁失敗後，舊捲動 observer 不會自動重試；保留現有結果並等待使用者重試。
- 時間軸正選景點更換時，尚未送出的舊備註不會寫進新景點；離開清單後接受邀請或複製分享，返回時會重新取得可存取的行程。
- 登入、註冊、驗證、密碼復原、邀請、授權、帳號與裝置管理補齊錯誤復原；OAuth 同意先驗證請求再保存授權。
- 健檢與 AI 筆記保留可辨識的工作狀態；健檢日期連結正確定位該天。巢狀對話框、桌機明細重新掛載、鍵盤操作與放大文字保留可用焦點及可讀內容。

### Changed
- 完成 #1307 的 38 項可靠性與無障礙驗收映射，保留核准的視覺設計；文件區分原生 HIG 參考與網頁無障礙要求。本版交付 UAT，正式環境上線另行處理。
...
### Fixed
- 首頁插畫恢復顯示：hero 的四個停留點、「每天的路線一眼看完」的兩條路線、「出發前先健檢」的進度條，自 v2.57.0 起因顏色變數沒有定義而整片隱形，現在淺色、深色都正常上色。
- 另外 5 處樣式引用了不存在的變數，改用既有 token：收藏加入行程的錯誤訊息恢復紅色底框、聊天「跳到最新」按鈕有實心底色、編輯行程點的備選標籤有底色，下拉更新失敗提示與協作頁標題的字級正確套用。
- 行程列印模式的「離開」按鈕重新套用最上層圖層：它引用的 z-index 變數在 v2.31.71 清理未使用 token 時被誤刪，按鈕可能被其他元素蓋住。

### Changed
- 新增檢查：樣式引用的變數沒有定義時，`npm test`（原始碼）與 e2e（build 後瀏覽器實際載入的樣式）都會失敗，同類問題不會再靜默上線。設計規範補記首頁插畫色的使用範圍；聊天「跳到最新」按鈕移除手機版重複的樣式規則（外觀不變）。
...
### Fixed
- **對外通道的健康檢查不再因為單一入口節點抖動就亂重設** —— Tailscale 會為對外網址發布多個入口節點，真正連線的一方（雲端觸發、瀏覽器）在第一個節點不通時會自動改走下一個。但健康檢查固定只探測其中第一個，那個節點一有延遲就判定整條通道壞掉並執行重設 —— 而重設當下通道才是真的短暫斷線。2026-09-01 單日累積 159 次誤判、38 次重設（其中 36 次重設後仍判定失敗）與 20 則通知。現在改成逐一探測每個入口節點，任一可通就算正常，全部不通才當作真的故障處理。

### Changed
- 健康檢查在「部分入口節點不通但服務仍可連」時會記下降級狀態，單一節點的長期劣化因此能被追蹤 —— 先前只探第一個節點，另一個節點的狀況完全不會出現在紀錄裡。
- 入口節點清單會先去除重複再逐一檢查，避免重複的 DNS 記錄讓單輪檢查時間成倍拉長、把警報拖過排程間隔。

### For contributors
- `funnel-guard` 的註解與 README 同步到多節點探測的新行為（舊敘述停留在「只取第一個位址」），並補上實測的時間預算公式與觸發上限。
...
### Added
- **聊天多了「跳到最新」箭頭** —— 往上翻歷史後，右下角會出現一個箭頭讓你一鍵回到最新訊息。以前翻上去就只能自己慢慢捲回來。
- **探索搜尋可以繼續往下載** —— 捲到結果底部會自動再載 20 筆，最多 60 筆；到底了會明說「已顯示全部 N 筆」。以前固定只給 20 筆，第 21 筆之後拿不到。

### Fixed
- **看歷史訊息時不會再被硬拉回底部** —— 以前你捲上去讀舊對話，只要有新訊息進來就會被強制拉到最下面。現在只有你本來就停在底部時才會自動跟著往下。
...
### Added
- **找回擱置四個月的行程筆記** —— 2026-03～04 寫的雨天備案與行程建議（共 67 筆，含各景點的室內替代方案、青之洞窟旺季預約前置期、餐廳實際營業時間）先前存在已退場的結構裡，App 完全看不到。現在搬進行程筆記的「雨天備案」與「行程建議」兩個區塊。⚠️ 它們登記為 AI 內容，**下次按「一般」AI 生成會被取代**；想永久保留某一筆，在 App 裡編輯它一次即可。

### Removed
- 行程筆記的舊儲存結構整條退場：每次開啟行程都會多打一次沒人使用的請求、每建一個新行程都會自動產生 5 筆空白紀錄，兩者都拿掉了。
- AI 助理不再能寫入該結構 —— 先前它「寫了但你看不到、還回報成功」的路徑已封死。

### Fixed
- 修正維運端點註解裡「每日檢查會自動呼叫」的過期說法（實際沒有；它是唯一的人工解鎖入口，不可刪）。

## [2.57.77] - 2026-07-28

### Added
- **行程 AI 聊天卡住時可以「停止等待」** —— 還在跑的訊息旁一直有停止鍵，按下去就放開輸入框、可以馬上再送下一句。誠實提醒：停止的是等待，不是 AI —— 它若已在動手，行程仍可能被改，完成後的回報還是會出現在對話裡。
- **沒人在處理的請求會被自動收掉** —— 排程器確定工作階段死掉時就地標記；就算整台排程機離線，聊天頁也會在停滯 100 分鐘後自己收屍。過去這種請求會永遠停在「處理中」，把整個聊天的輸入框鎖死。

### Fixed
- AI 完成得比停止晚時，它的回報不再被整包丟掉 —— 內容會補進對話，但不會讓已終結的請求復活。
- 使用者自己按的停止改用中性樣式，不再畫成紅色錯誤。
...
- **既有債重新分類（不只是重數）**：先前寫「兩套各 15 條、B 散在 7 個元件」。逐條看過後，把 38 條 `outline: none` 整批當成同一種東西**本身就是錯的分類** —— 拆開是 A（`--shadow-ring`）20、**C（`border-color` + 極淡 `accent-subtle` 光暈）8 —— 本檔從未記載過的第三套**、C′（只有 border-color）2、**highlight 3（不是債，正好是「清單／集合用 highlight」的現成先例）**、刻意抑制 3、表單例外 1。**真正要遷的是 30 條，不是 38 條。**
- **慣例 B 自己也有失效模式**：「`outline-offset` 天生有間隙」只對**正值**成立。全庫 38 條裡有 **6 條負值**（`-2px`×4、`-3px`×2）把框畫進元件自己的 padding box，相鄰色又變回自家底色 —— 跟慣例 A 同一個失效模式。規則表的「機制」列已補上 offset 必須為正。
- **差點誤報一條**：`.tp-rail-note-input:focus { outline: none; }` 看起來是「拿掉指示又不補」，但那個 textarea 只在編輯態掛載、恆有 accent 框 + 光暈，落在 §Accessibility 的表單輸入例外裡。已標明不是違規。
...
### Fixed
- **行程一覽還原上次檢視的行程與位置**（進 `/trips` 未帶 `?selected` 時）— 桌機記住上次看的行程 + 天（`dayNum`），回來直接停在原處，不用重選（`tripViewState` localStorage SoT，6 個月 TTL、壞資料 fail-closed；手機不還原）。
- **桌機景點編輯頁改備註返回時顯示新值**（原本 SPA 返回顯 stale、要 F5 才更新）— 根因是 `useAutosave.flush()` 撞到 onBlur 已觸發的 in-flight PATCH 時會即刻 return，返回的 days GET 讀到舊值。`flush()` 補上真正的 barrier：await 進行中的存檔完成後才返回 → 返回必見新備註。
- **編輯景點回前頁回到當下景點展開** — `goBack` 帶 `?focus=<entryId>`，`TimelineRail` 掛載即展開該景點（只認屬於本 rail 的 entry），只關編輯窗、停在原景點。
- **手機備註輸入不再誤觸整頁 reload** — `usePullToRefresh` 加 editable-target guard，點 `input`/`textarea`/contenteditable 不再被當成下拉更新手勢。
