...
The Web chat uses the approved inline card (prototype C) above the composer. It preserves the draft on refusal or failure and continues the attempted send only after a successful acceptance. When consent is current, “管理 AI 資料同意” opens the same card to revoke it. The approved C extension places the same card beside AI actions in trip health checks and trip notes. A gated action resumes only after the server confirms current acceptance; refusal leaves the report or notes unchanged. The server gate remains authoritative if consent changes on another device.
...
If the submitter lacks current acceptance, the three AI mutation endpoints return HTTP 403 `{ "error": { "code": "AI_DATA_CONSENT_REQUIRED" } }`. If the submitter is current but the trip owner is not, they return HTTP 403 with code `AI_DATA_CONSENT_OWNER_REQUIRED`; the client should ask the owner to accept and must not loop the submitter through their own consent card. A queued request already accepted by the API can later become `status: "failed", terminalReason: "needs_consent"` after the mint recheck; clients should refresh consent and offer a new request after the missing party accepts.
