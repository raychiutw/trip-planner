...
### 3c-bis. Dry-run mode（DX-C4 escape hatch）

旅伴 / Ray 可在 message 加 metadata 觸發 dry-run（測試 LLM 解析正確性，不實際寫入）：

- **如何啟用**：呼叫 PATCH /api/requests/{id} 時帶 header `X-Request-Dry-Run: 1` (companion scope only)。
- **行為**：執行步驟 3c 判斷 + reply 寫「我會做 X / Y」摘要，但 **不打** 任何寫入 API（POST entries / PATCH entries / POST trip-pois / etc.）。
- **用途**：Ray 排查 LLM 誤判時不污染 prod 資料。message 解析錯了，dry-run reply 顯示「我會...」但實際沒做 → 修 SKILL.md rubric。
