...
0. 執行 `git status --short`，記錄 pre-existing 未暫存變更（後續 Step 8 僅還原本次新增的非白名單檔案，不觸碰既有變更）
...
6. 若插入、移除或移動 entry，重新估算相鄰 travel 的 type + 分鐘數
7. 執行 `npm run build` 更新 dist
8. 執行 `git diff --name-only`，與 Step 0 記錄比對：
